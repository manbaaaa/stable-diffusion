...
class DDPMSampler(nn.Module):
    ...
    def set_inference_steps(self, num_inference_steps: int = 50):
        ...
        self.ratio = self.num_training_steps // self.inference_steps
        timesteps = (
            (np.arange(self.inference_steps) * ratio)
            .round()[::-1]
            .copy()
            .astype(np.int64)
        )
    ...
    def _get_previous_timesteps(self, timesteps: int) -> int:
        prev_t = timesteps - self.ratio
        return prev_t

    def _get_variance(self, timesteps: int) -> torch.Tensor:
        prev_t = self._get_previous_timesteps(timesteps)
    ...
    def set_strength(self, strength: float = 1.0):
        ...
        """
        # strat_step is the number of steps to start adding noise
        start_step = self.num_inference_steps - (self.num_inference_steps * strength)
        self.timesteps = self.timesteps[start_step:]
        self.start_step = start_step

    def step(
        self, timestep: int, latent: torch.Tensor, model_output: torch.Tensor
    ) -> torch.Tensor:
        t = timestep
        ...
        predicted_original_sample = (
            latent - beta_prod_t**0.5 * model_output / alpha_prod_t**0.5
        )
...
def add_noise(
    self, origin_samples: torch.FloatTensor, timesteps: torch.IntTensor
) -> torch.FloatTensor:
    alpha_cumprod = self.alpha_cumprod.to(
        device=origin_samples.device, dtype=origin_samples.dtype
    )
    timesteps = timesteps.to(device=origin_samples.device)
    ...
    noise = torch.randn_like(origin_samples, generator=self.generator)
